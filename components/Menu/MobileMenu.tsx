...
const MobileMenu = () => {
    ...
    return (
        <div className=' z-50 flex md:hidden right-0 fixed h-[50px] bg-primary bottom-0 w-full'>
            <div className='flex items-center justify-evenly w-full'>
                {menuItems.map((data, index) => (
                    <div key={index} onClick={() => router.push(data.path)} className={`flex gap-2 justify-start items-center  cursor-pointer hover:bg-white hover:text-black rounded-[50%] p-1 ${pathname == data.path ? 'bg-white text-black ' : 'text-white'}`}>
                        {data.icon}
