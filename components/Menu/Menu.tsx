...
const Menu = () => {
  ...
  return (
    <div className={`${parentOpen ? 'w-[300px]' : 'w-[80px] '} hidden md:block  h-screen duration-200 py-1 pl-1     text-black/80`}>
      <div className=' rounded h-full bg-white border border-black/15'>
        <div className={`flex ${open ? 'justify-between  p-5' : 'justify-center  p-2 text-[25px] text-center'} items-center border-b border-black/15`}>
        ...
        <div className={`flex flex-col justify-between gap-10  h-[90vh] ${open ? 'p-5' : 'p-0 py-5 justify-between items-center '}`}>
          <div className='flex flex-col gap-4'>
            {menuItems.map((data, index) => (
              <div key={index} onClick={() => router.push(data.path)} className={`flex gap-2 justify-start items-center cursor-pointer hover:bg-primary hover:text-white ${open ? 'rounded' : 'rounded-[50%]'} p-2 ${pathname == data.path && 'bg-primary text-white'}`}>
                {data.icon}
